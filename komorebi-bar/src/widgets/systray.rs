...
impl Default for GlobalSystrayState {
    fn default() -> Self {
        Self {
            icons: HashMap::new(),
            event_rx: None,
        ...
        rt.block_on(async move {
            match SystrayClient::new() {
                Ok(mut systray) => {
                    ...
                    let (async_cmd_tx, mut async_cmd_rx) = tokio::sync::mpsc::unbounded_channel::<SystrayCommand>();
...
fn process_events() -> bool {
    ...
    fn create_cached_icon(icon: &SystrayIcon) -> CachedIcon {
        ...
        let exe_name = icon
            .window_handle
            .and_then(|hwnd| Self::get_exe_from_hwnd(hwnd))
            .unwrap_or_default();
    ...
    fn send_action(stable_id: &StableId, action: SystrayIconAction) {
        ...
        if let Some(command_tx) = &state.command_tx {
            if command_tx.send(SystrayCommand::SendAction(stable_id.clone(), action)).is_err() {
    ...
    fn is_icon_hidden(&self, exe_name: &str) -> bool {
        ...
        self.hidden_icons.iter().any(|hidden| *hidden == exe_lower)
    }
}
        ...
        fn render(&mut self, ctx: &Context, ui: &mut Ui, config: &mut RenderConfig) {
            ...
            config.apply_on_widget(false, ui, |ui| {
                ...
                if has_hidden_icons && effective_toggle_left {
                    self.render_toggle_button(ui, show_hidden, &hidden_icons, icon_size, ctx, is_reversed);
                }
